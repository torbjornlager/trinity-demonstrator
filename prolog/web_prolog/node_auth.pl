...
:- use_module(node_tokens, [request_bearer_principal/2]).

:- setting(auth, atom, open, 'Authorization mode: open, private, or dev').
:- setting(authenticated_default_capabilities, list(atom), [],
           'The "registered" tier: capabilities for an authenticated principal not listed in the principal policy. Empty = off (such principals stay unknown and are denied).').
:- setting(anon_per_ip, boolean, false,
           'Individualise the anonymous principal per client IP, so the shared anonymous identity is not one rate/limit/audit bucket for the whole internet. Off = the shared `anonymous` principal.').
...
request_principal(Request, Principal) :-
    ...
    ;   anonymous_principal(Request, Principal)
    ).


%!  anonymous_principal(+Request, -Principal) is det.
%
%   The shared `anonymous` atom by default. When `anon_per_ip` is on,
%   individualise it per client IP (`anon:<ip>`) so the anonymous
%   identity does not collapse into a single rate / per-principal-limit /
%   audit bucket for the whole internet — the /call analogue of
%   `anon_per_ws_connection`. The IP is resolved spoof-resistantly (see
%   client_ip/2), so a client cannot evade or steal another's bucket.
anonymous_principal(Request, Principal) :-
    (   anon_per_ip_enabled,
    ...
    ->  auth_mode(Mode),
        ...
        format(string(Id), "anon:~w", [IP]),
        Principal = principal{id:Id, capabilities:Capabilities, unknown:false}
    ;   Principal = anonymous
    ).

anon_per_ip_enabled :-
    (   catch(current_node_value(anon_per_ip, Value), _, fail)
    ->  Value == true
    ;   setting(anon_per_ip, true)
    ).
...
%  A proven bearer token wins over the (proxy-trusted) identity header:
%  it carries a secret, so it is safe even on a directly exposed node.
%  An absent or invalid token simply falls through to the header path,
%  then to anonymous — a bad token is unauthenticated, not a hard error.
...
%   boundary as the internal transport. A bearer token needs no such gate
%   (it carries a proven secret) and is tried first.
request_forwarded_identity_trusted(Request) :-
...
authenticated_principal_policy(PrincipalId, Capabilities, Unknown) :-
    ...
    ->  Capabilities = PolicyCapabilities,
        Unknown = false
    ;   authenticated_default_capabilities(DefaultCapabilities),
        DefaultCapabilities \== []
    ->  %  The "registered" tier: an authenticated identity (e.g. an SSO
        %  user verified by the proxy) that has no explicit policy entry
        %  gets these default capabilities, while keeping its own id for
        %  audit and per-principal rate limiting.
...
%!  authenticated_default_capabilities(-Capabilities) is det.
%
%   The configured "registered" tier (a list of capabilities). Read from
%   the node runtime value if present, else the setting, else empty.
%   Empty means the tier is off and authenticated-but-unconfigured
%   principals stay unknown — the default, so existing behavior is
%   unchanged unless an operator opts in.
authenticated_default_capabilities(Capabilities) :-
    (   current_node_value(authenticated_default_capabilities, Caps0)
    ->  true
    ;   setting(authenticated_default_capabilities, Caps0)
    ),
    %  Never grant admin or internal_transport as a blanket tier — those
    %  are per-principal only. Defense in depth: start_node also rejects
    %  them at config time, but filter here whatever the source.
    exclude(unsafe_tier_capability, Caps0, Capabilities).

unsafe_tier_capability(admin).
unsafe_tier_capability(internal_transport).
...
%!  client_ip(+Request, -IP:string) is det.
%
%   Spoof-resistant client IP. X-Forwarded-For is honoured ONLY when the
%   peer is a trusted front end (loopback / private network — the reverse
%   proxy), and then the *rightmost* entry is taken (the address the
%   trusted proxy actually observed; a client prepending its own value
%   cannot shift it). A directly-connected client's X-Forwarded-For is
%   ignored and its real peer address is used. (node_log's request_peer/2
%   trusts the header unconditionally — fine for best-effort attribution,
%   not for an access/identity decision, hence this resolver.) Used by the
%   IP/CIDR access control and the per-IP anonymous identity.
client_ip(Request, IP) :-
    (   client_ip_trusted_proxy(Request),
        forwarded_for_client(Request, IP0)
    ->  IP = IP0
    ;   request_peer_ip(Request, IP)
    ->  true
    ;   IP = ""
    ).
...
forwarded_for_client(Request, IP) :-
    (   memberchk(x_forwarded_for(Value), Request)
    ->  true
    ;   memberchk('x-forwarded-for'(Value), Request)
    ),
    ip_text(Value, Text),
    split_string(Text, ",", " \t", Parts0),
    exclude(==(""), Parts0, Parts),
    Parts \== [],
    last(Parts, IP).
...
request_peer_ip(Request, IP) :-
    memberchk(peer(Peer), Request),
    peer_ip_text(Peer, IP).

peer_ip_text(Host:_Port, IP) :-
    !,
    peer_ip_text(Host, IP).
peer_ip_text(ip(A, B, C, D), IP) :-
    !,
    format(string(IP), "~w.~w.~w.~w", [A, B, C, D]).
peer_ip_text(ip(A, B, C, D, E, F, G, H), IP) :-
    !,
    format(string(IP), "~16r:~16r:~16r:~16r:~16r:~16r:~16r:~16r",
           [A, B, C, D, E, F, G, H]).
peer_ip_text(Other, IP) :-
    ip_text(Other, IP).
