...
profile_check_qualified_goal(Profile, actor, Goal) :-
    ...
    ensure_goal_profile(Profile, Goal).
profile_check_qualified_goal(Profile, actors, Goal) :-
    !,
    ensure_public_actor_goal(Goal),
    ensure_goal_profile(Profile, Goal).
profile_check_qualified_goal(Profile, toplevel_actor, Goal) :-
    !,
    ensure_goal_profile(Profile, Goal).
profile_check_qualified_goal(Profile, toplevel_actors, Goal) :-
    !,
    ensure_goal_profile(Profile, Goal).
...
ensure_public_actor_goal(Goal) :-
    ...
    predicate_property(actors:Head, exported),
    !.
ensure_public_actor_goal(Goal) :-
    %  The error term is the demonstrator's exact observable shape;
    %  only the predicate_property check above follows the module
    %  rename.
