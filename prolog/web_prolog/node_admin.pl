...
:- use_module(node_auth, [
    ...
    authenticated_default_capabilities/1,
    anon_per_ip_enabled/0
]).
:- use_module(node_ip_policy, [
    current_ip_allowlist/1,
    current_ip_blocklist/1,
    auto_ban_threshold/1,
    auto_ban_window_seconds/1,
    auto_ban_seconds/1
]).
:- use_module(node_relation_policy, [relation_patterns/1]).
:- use_module(rpc, [text_to_string/2, normalize_timeout/2]).
...
:- use_module(node_log, [
    ...
    current_log_capacity/1,
    current_log_retention_seconds/1
]).
:- use_module(node_interaction_log, [log_interaction_request/2]).
:- use_module(node_doctor, [node_doctor_report/2]).
...
%!  node_admin_tabler_css_page(+Request) is det.
%
%   Serve the vendored Tabler stylesheet for the admin panel. Static,
%   no secrets, so it is not admin-gated.
node_admin_tabler_css_page(Request) :-
    node_admin_asset_file('../../web/tabler.min.css', File),
    http_reply_file(File, [unsafe(true), mime_type('text/css; charset=UTF-8')], Request).
...
%!  node_admin_maintenance_page(+Request) is det.
%
%   Headless control of maintenance/drain mode.  GET reports the
%   current state; POST {"enabled":Bool} sets it.  While enabled the
%   node refuses new execution work and `/readyz` returns 503, so a
%   load balancer drains it (the operator then stops the process — see
%   the SIGTERM handler in Deployment/start_node.pl — which graceful-
%   drains and exits).
node_admin_maintenance_page(Request) :-
    with_node_request_context(
        Request,
        node_admin_reply(
            admin_maintenance_response(Request)
        )
    ).

admin_maintenance_response(Request, json{maintenance:State}) :-
    require_admin_principal(Request),
    request_method(Request, Method),
    (   Method == post
    ->  read_admin_json_dict(Request, Dict),
        get_dict(enabled, Dict, Enabled),
        must_be(boolean, Enabled),
        current_node_port(Port),
        set_node_maintenance(Port, Enabled),
        ignore(catch(log_admin_event(Request, "admin_maintenance",
                                     _{status:"success", enabled:Enabled}),
                     _, true))
    ;   Method == get
    ->  true
    ;   throw(error(domain_error(http_method, Method),
                    context(node_admin:node_admin_maintenance_page/1,
                            'admin maintenance endpoint only supports GET and POST')))
    ),
    current_node_maintenance(State).


%!  node_admin_tokens_page(+Request) is det.
%
%   Bearer-token administration.  GET lists tokens (no secrets/hashes);
%   POST issues one — `{"principal":..., "capabilities":[...],
%   "expires_in":Secs?, "label":...?}` — and returns the full token
%   ONCE (it is unrecoverable afterwards); DELETE ?id=<id> revokes one.
node_admin_tokens_page(Request) :-
    with_node_request_context(
        Request,
        node_admin_reply(
            admin_tokens_response(Request)
        )
    ).

admin_tokens_response(Request, JSON) :-
    require_admin_principal(Request),
    request_method(Request, Method),
    (   Method == get
    ...
    ;   Method == post
    ->  read_admin_json_dict(Request, Dict),
        admin_issue_token(Dict, JSON),
        get_dict(id, JSON, IssuedId),
        get_dict(principal, Dict, IssuedPrincipal),
        ignore(catch(log_admin_event(Request, "admin_token_issue",
                                     _{status:"success",
                                       token_id:IssuedId,
    ...
    ->  http_parameters(Request, [id(Id, [])]),
        ...
        current_tokens(Tokens),
        JSON = json{revoked:Revoked, id:Id, tokens:Tokens},
        ignore(catch(log_admin_event(Request, "admin_token_revoke",
                                     _{status:"success", token_id:Id,
                                       revoked:Revoked}),
                     _, true))
    ;   throw(error(domain_error(http_method, Method),
                    context(node_admin:node_admin_tokens_page/1,
                            'admin tokens endpoint supports GET, POST, DELETE')))
    ).

admin_issue_token(Dict, json{token:Full, id:Id, tokens:Tokens}) :-
    (   get_dict(principal, Dict, Principal0)
    ->  text_to_string(Principal0, Principal)
    ;   throw(error(domain_error(token_field, principal),
                    context(node_admin:admin_issue_token/2,
                            'issuing a token requires a "principal"')))
    ),
    Principal \== "",
    (   get_dict(capabilities, Dict, Capabilities)
    ->  true
    ;   Capabilities = [execute]
    ),
    admin_token_options(Dict, Options),
    issue_token(Principal, Capabilities, Options, Full),
    split_string(Full, "_", "", ["wp", IdString, _]),
    atom_string(Id, IdString),
    current_tokens(Tokens).

admin_token_options(Dict, Options) :-
    findall(Option, admin_token_option(Dict, Option), Options).

admin_token_option(Dict, expires_in(Seconds)) :-
    get_dict(expires_in, Dict, Seconds),
    number(Seconds).
admin_token_option(Dict, label(Label)) :-
    get_dict(label, Dict, Label0),
    text_to_string(Label0, Label).


%!  node_admin_doctor_page(+Request) is det.
%
%   Self-diagnostics: a green/amber/red review of the node's security and
%   operational posture (sandbox, auth, ceilings, TLS, token/log
%   durability, maintenance). GET, admin-gated, read-only.
node_admin_doctor_page(Request) :-
    with_node_request_context(
        Request,
        node_admin_reply(
            admin_doctor_response(Request)
        )
    ).

admin_doctor_response(Request, JSON) :-
    require_admin_principal(Request),
    request_method(Request, Method),
    (   Method == get
    ->  node_doctor_report(Request, JSON)
    ;   throw(error(domain_error(http_method, Method),
                    context(node_admin:node_admin_doctor_page/1,
                            'admin doctor endpoint only supports GET')))
    ).


%!  node_admin_posture_page(+Request) is det.
%
...
node_admin_posture_page(Request) :-
    with_node_request_context(
        Request,
        node_admin_reply(
            admin_posture_response(Request)
        )
    ).

admin_posture_response(Request, JSON) :-
    require_admin_principal(Request),
    request_method(Request, Method),
    (   Method == get
    ->  current_admin_posture(JSON)
    ;   throw(error(domain_error(http_method, Method),
                    context(node_admin:node_admin_posture_page/1,
                            'admin posture endpoint only supports GET')))
    ).
...
node_admin_file(File) :-
    node_admin_asset_file('../../web/admin.html', File).

node_admin_asset_file(RelPath, File) :-
...
%!  current_admin_posture(-JSON) is det.
%
%   Read-only security/operations posture: settings configured outside the
%   editable Config form (or startup-only). All have safe-by-default values
%   (empty lists, 0 thresholds, false flags) so a freshly started node
%   reports an empty/off posture.
current_admin_posture(json{
    authenticated_default_capabilities:AuthDefaultCaps,
    anon_per_ip:AnonPerIP,
    load_uri_allowed_origins:LoadUriOrigins,
    relation_patterns:RelationPatterns,
    ip_allowlist:IpAllow,
    ip_blocklist:IpBlock,
    auto_ban_threshold:BanThreshold,
    auto_ban_window_seconds:BanWindow,
    auto_ban_seconds:BanSeconds,
    log_capacity:LogCapacity,
    log_retention_seconds:LogRetention
}) :-
    authenticated_default_capabilities(AuthDefaultCaps),
    (   anon_per_ip_enabled
    ->  AnonPerIP = true
    ...
    (   current_node_value(load_uri_allowed_origins, LoadUriOrigins)
    ->  true
    ;   LoadUriOrigins = []
    ),
    relation_patterns(RelationPatterns0),
    maplist(relation_pattern_text, RelationPatterns0, RelationPatterns1),
    list_to_set(RelationPatterns1, RelationPatterns),
    current_ip_allowlist(IpAllow),
    current_ip_blocklist(IpBlock),
    auto_ban_threshold(BanThreshold),
    auto_ban_window_seconds(BanWindow),
    auto_ban_seconds(BanSeconds),
    current_log_capacity(LogCapacity),
    current_log_retention_seconds(LogRetention).

%!  relation_pattern_text(+Pattern, -Text) is det.
%
%   Render an advertised RELATION pattern as a readable string with named
%   variables (e.g. `parent(A, B)`), so it survives JSON encoding.
relation_pattern_text(Pattern0, Text) :-
    copy_term(Pattern0, Pattern),
    numbervars(Pattern, 0, _),
    format(string(Text), "~W", [Pattern, [numbervars(true), quoted(true)]]).
...
current_admin_config(json{
    ...
    %  Resource ceilings + log rotation (Phase 8). Read-only in the
    %  admin surface for now — startup-configured; 0 means unlimited/off.
    config_ceiling(max_actors, MaxActors),
    config_ceiling(max_call_inferences, MaxCallInferences),
    config_ceiling(max_actor_stack_bytes, MaxActorStackBytes),
    config_ceiling(max_interaction_log_bytes, MaxLogBytes),
    config_int(max_interaction_log_backups, 5, MaxLogBackups),
    current_builtin_families_json(BuiltinFamilies).

%!  config_ceiling(+Key, -Value) is det.
%
%   A resource ceiling as a non-negative integer; the `unlimited` atom
%   and any unset/non-integer value report as 0 (= unlimited/off).
config_ceiling(Key, Value) :-
    (   current_node_value(Key, V), integer(V), V > 0
    ->  Value = V
    ;   Value = 0
    ).

config_int(Key, Default, Value) :-
    (   current_node_value(Key, V), integer(V)
    ->  Value = V
    ;   Value = Default
    ).
...
log_admin_event(Request, Action0, Fields0) :-
    ...
    %  In-memory log for the live admin dashboard.
    ignore(catch(log_event(Event), _, true)),
    %  Durable audit trail: persist the mutation (who/what/when) to the
    %  append-only interaction log, where it survives restarts and is
    %  visible in the secret viewer (event names are prefixed "admin:").
    ignore(catch(log_admin_audit(Request, Action, Fields0), _, true)).

log_admin_audit(Request, Action, Fields0) :-
    format(string(EventName), "admin:~w", [Action]),
    put_dict(_{event:EventName, action:Action}, Fields0, AuditEvent),
    log_interaction_request(Request, AuditEvent).
