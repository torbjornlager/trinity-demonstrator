...
:- dynamic start_n4_done/0.

:- initialization(start_n4).
...
start_n4_service_bootstrap :-
    thread_create(
            ...
            with_node_port_context(3055, ensure_example_services_started),
            Error,
...
ensure_example_services_started(0) :-
    throw(error(resource_error(node_resident_services),
                context(start_n4/0,
                        'unable to register counter and pubsub_service on n4'))).
