...
/** <module> Source and URI Helpers

Small utility predicates shared by several modules (`actor`, `node`,
`statechart_actor`) for turning different source representations into plain
text that can be loaded/consulted.

Supported source forms include:

  - a list of Prolog terms (serialized to source text),
  - a list of predicate indicators (`Name/Arity`) listed from a module,
  - local files (plain path or `file://` URI),
  - remote HTTP(S) URIs.

The intent is to keep all URI/path normalization logic in one place so that
loading behavior is consistent across the PoC.
*/
...
:- use_module(pid_utils, [registered_self_node_url/1]).
...
%     - a node-relative HTTP(S) URI based on the registered node self URL.
%
...
resolve_node_relative_uri(Rel0, URI) :-
    registered_self_node_url(Base0),
    !,
