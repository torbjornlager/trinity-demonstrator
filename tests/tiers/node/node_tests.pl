...
:- use_module('../../../prolog/web_prolog/node_auth.pl', [
    auth_mode/1,
...
:- use_module('../../../prolog/web_prolog/node_capabilities.pl', [
...
:- use_module('../../../prolog/web_prolog/node_response.pl', [answer_to_json/2]).
...
:- use_module('../../../prolog/web_prolog/goal_walker.pl', [walk_goal/2]).
:- use_module('../../../prolog/web_prolog/public_goal_guard.pl', [
    rewrite_goal_if_needed/3,
    rewrite_source_text_if_needed/3
]).
:- use_module('../../../prolog/web_prolog/node_session.pl', [cleanup_isotope_session/1]).
:- use_module('../../../prolog/web_prolog/node_runtime_state.pl', [
...
%!  wait_until(:Goal, +Attempts) is semidet.
%
%   Poll Goal until it succeeds, up to Attempts times (10 ms apart).
%   For test assertions that race an asynchronous side effect (e.g. a
%   session teardown that completes just after its WS reply is sent):
%   bound the wait instead of checking the instant the reply arrives.
wait_until(Goal, _) :-
    call(Goal),
    !.
wait_until(_, Attempts) :-
    Attempts =< 0,
    !,
    fail.
wait_until(Goal, Attempts) :-
    sleep(0.01),
    Remaining is Attempts - 1,
    wait_until(Goal, Remaining).
...
test(node_portal_and_example_routes_served) :-
    with_node_server(URI,
        (
            ...
            format(atom(JevTutorialURL), '~w/jev-sxml-tutorial', [URI]),
            read_text(JevTutorialURL, JevTutorialBody),
            ...
            format(atom(JevActorExampleURL), '~w/examples/actors/23%20jev-agent.pl', [URI]),
            read_text(JevActorExampleURL, JevActorExampleBody),
            format(atom(JevSxmlActorExampleURL), '~w/examples/actors/24%20jev-sxml-agent.pl', [URI]),
            read_text(JevSxmlActorExampleURL, JevSxmlActorExampleBody),
            ...
            format(atom(SxmlDTDURL), '~w/sxml-0.2.dtd', [URI]),
            read_text(SxmlDTDURL, SxmlDTDBody),
            format(atom(SwiWasmWorkerURL), '~w/swi_wasm_actor_worker.js', [URI]),
            read_text(SwiWasmWorkerURL, SwiWasmWorkerBody),
            format(atom(WasmSharedDBURL), '~w/wasm/shared_db.pl', [URI]),
            read_text(WasmSharedDBURL, WasmSharedDBBody),
            format(atom(SxmlSchemaURL), '~w/wasm/sxml_schema.pl', [URI]),
            read_text(SxmlSchemaURL, SxmlSchemaBody),
            ...
            assertion(sub_string(DemonstratorBody, _, _, _, 'Jev agent experiment')),
            ...
            assertion(sub_string(DemonstratorBody, _, _, _, '@click="runActorEditorGoal(''flush.'')"')),
            assertion(sub_string(DemonstratorBody, _, _, _, '@click="runActorEditorGoal(''actors(Actors).'')"')),
            assertion(sub_string(DemonstratorBody, _, _, _, 'v-if="effectiveTransportProfile === ''actor''"')),
            ...
            assertion(sub_string(ActorTutorialBody, _, _, _, 'function installTutorialHeadingLinks(headings)')),
            assertion(sub_string(ActorTutorialBody, _, _, _, 'button.className = "heading-link"')),
            assertion(sub_string(ActorTutorialBody, _, _, _, 'button.textContent = "#"')),
            assertion(sub_string(ActorTutorialBody, _, _, _, 'installTutorialHeadingLinks(headings);')),
            assertion(sub_string(TutorialCSSBody, _, _, _, '.heading-link')),
            ...
            assertion(sub_string(ActorTutorialBody, _, _, _, 'id="tutorial-programming-with-statecharts"')),
            assertion(sub_string(ActorTutorialBody, _, _, _, 'id="sc-pr-spawn"')),
            assertion(sub_string(ActorTutorialBody, _, _, _, 'id="sc-sp-diagram"')),
            assertion(sub_string(ActorTutorialBody, _, _, _, 'id="sc-em-diagram"')),
            assertion(sub_string(ActorTutorialBody, _, _, _, 'var SC_CHARTS =')),
            assertion(sub_string(ActorTutorialBody, _, _, _, 'function scSpawnDialog(exampleId)')),
            ...
            assertion(sub_string(JevActorExampleBody, _, _, _, 'jev_agent_spawn')),
            assertion(sub_string(JevSxmlActorExampleBody, _, _, _, 'jev_sxml_agent_spawn')),
            assertion(sub_string(ServiceExampleBody, _, _, _, 'pubsub_actor')),
            assertion(sub_string(StatechartBody, _, _, _, '<statechart')),
            assertion(sub_string(SxmlDTDBody, _, _, _, '<!ELEMENT statechart')),
            ...
            assertion(sub_string(SxmlSchemaBody, _, _, _, ':- module(sxml_schema')),
            ...
            memberchk(_{name:"04 count_server.pl", url:"/examples/actors/04 count_server.pl", kind:"prolog"},
                      ActorEntries),
            memberchk(_{name:"23 jev-agent.pl", url:"/examples/actors/23 jev-agent.pl", kind:"prolog"},
                      ActorEntries),
            memberchk(_{name:"24 jev-sxml-agent.pl", url:"/examples/actors/24 jev-sxml-agent.pl", kind:"prolog"},
                      ActorEntries),
            memberchk(_{name:"20 jev-deployment-assistant.xml", url:"/examples/statecharts/20 jev-deployment-assistant.xml", kind:"statechart"},
                      StatechartEntries),
            ...
            assertion(sub_string(StatechartTutorialBody, _, _, _, 'the ACTOR profile')),
            assertion(sub_string(StatechartTutorialBody, _, _, _, 'id="tutorial-programming-with-statecharts"')),
            assertion(sub_string(StatechartTutorialBody, _, _, _, 'id="sc-pr-diagram"')),
            assertion(sub_string(StatechartTutorialBody, _, _, _, 'id="sc-spaghetti-spawn"')),
            assertion(sub_string(StatechartTutorialBody, _, _, _, 'id="sc-emotions-spawn"')),
            assertion(sub_string(StatechartTutorialBody, _, _, _, 'id="sc-dialog-spawn"')),
            assertion(sub_string(StatechartTutorialBody, _, _, _, 'id="sc-spawn-toplevel-spawn"')),
            assertion(sub_string(StatechartTutorialBody, _, _, _, 'bridge.onStatechartTrace = function(text)')),
            assertion(sub_string(StatechartTutorialBody, _, _, _, '/vendor/tutorial-common.css')),
            assertion(sub_string(JevTutorialBody, _, _, _, 'Jev semantic observations in Web Prolog')),
            assertion(sub_string(JevTutorialBody, _, _, _, 'We already have a complete neuro-symbolic')),
            assertion(sub_string(JevTutorialBody, _, _, _, 'id="jev-swedish"')),
            assertion(sub_string(JevTutorialBody, _, _, _, '23%20jev-agent.pl')),
            assertion(sub_string(JevTutorialBody, _, _, _, '24%20jev-sxml-agent.pl')),
            assertion(sub_string(JevTutorialBody, _, _, _, 'data-load-target="session"')),
            assertion(sub_string(DemonstratorBody, _, _, _, 'preserveWorkspace: true')),
...
test(node_admin_and_portal_pages_include_manual_predicate_link_support) :-
    ...
    %  node.pl lives one level deeper than in the demonstrator.
    ...
    with_node_server_options([auth(private), owner("owner"), AlicePolicy], URI,
        setup_call_cleanup(
            ...
            (
                ...
                catch(actors:exit(ActorPid, kill), _, true),
    ...
    setup_call_cleanup(
        ...
        with_node_port_context(Port,
            (
                ...
                actors:spawn(sleep(10), Pid),
                actors:monitor(Pid, Ref),
    ...
    with_node_server_options(
        ...
        setup_call_cleanup(
            ...
            (
                ...
                (   nonvar(Pid1)
                ->  catch(actors:exit(Pid1, kill), _, true)
                ;   true
                )
            )
        )).
            ...
            with_node_server(PingNode,
                setup_call_cleanup(ws_open(Home, WS),
                    ( % Use the example's actual predicates without changing
                      ...
                      directory_file_path(TestDir, '../../../examples/actors/06 ping-pong.pl', Example),
    ...
    with_node_server(URI,
        setup_call_cleanup(ws_open(URI, WS),
            ( ws_send_json(WS, json{command:transport_hello,version:1,browser_pids:true}),
              ...
              once((member(Message, Replies), Message.type == "actor_message")),
    ...
    with_node_server(URI,
        setup_call_cleanup(ws_open(URI, WS),
            ( ws_send_json(WS, json{command:transport_hello,version:1,
              ...
              once((member(Spawned, First), Spawned.type == "spawned")),
              once((member(Ping, First), Ping.type == "actor_message")),
              ...
              once((member(Finished,Last), Finished.type == "actor_message")),
              assertion(Finished.message == "finished"),
              once((member(Output,Last), Output.type == "io_request")),
...
test(ws_send_to_other_connections_registered_name_is_rejected,
    ...
    %  actors:whereis/2 -- on its face this looks like it widens the
    %  ownership check to "any registered actor on this node", but
    %  actors:whereis/2 is scoped to current_registry_namespace/1, and
...
test(actor_node_url_to_ws_endpoint_honors_runtime_override,
    ...
    setup_call_cleanup(
        ...
        once(node_runtime_state:with_node_port_context(
            3053,
            distribution:node_url_to_ws_endpoint('https://n4.example', WsURL)
        )),
        retractall(node_runtime_state:node_runtime(_, _))
    ).
...
test(profile_check_goal_rejects_non_exported_actor_parent_predicate,
     [throws(error(profile_violation(_, _), _))]) :-
    profile_check_goal(actor, actors:'$parent'(_Parent)).
