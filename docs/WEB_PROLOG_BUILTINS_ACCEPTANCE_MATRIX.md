...
| `toplevel_spawn/1-2`, `toplevel_call/2-3`, `toplevel_next/1-2`, `toplevel_stop/1`, `toplevel_abort/1`, `toplevel_halt/1-2` | no | no | yes | These are accepted as client goals only in the `actor` profile. The HTTP `/toplevel_*` endpoints are separate host-side routes. |
