...
| **self-reported** | the node's own `/info` endpoint | true if the node is honest; verifiable | `profile`, `auth`, `version`, `services`, `provides` |
...
% --- self-reported tier (from /info, refreshed each probe) ---
...
- `profile`, `auth` — straight from `node_info_page_1/1` in
  [node.pl](../src/node.pl) (around line 1216), which already returns
  `self_url`, `profile`, `auth`, and the trusted-header policy.
- `version` — already at the `/version` page (`web_prolog: "0.2.0"`).
...
- `services` — the node tracks these (`whereis_service/2` in
  ...
  them today, so harvesting them needs a small `/info` addition (or a new
  endpoint).  It is therefore *not* free: `node_service/2` defers alongside
  `provides`, past slice 1 (§9).
...
`profile` and `auth` from `/info`, `version` from `/version` — and nothing
else.  `services` and `provides` arrive together once a node-side endpoint
publishes them.
...
- **transient probers** — spawned per node per sweep, do the one blocking
  `/info` fetch, report the result back as a message, and die.  A slow
...
- `ok(Info)` arrives → **overwrite the self-reported tier** from `Info`;
  ...
  **leave the seed tier untouched**.  In slice 1 the self-reported tier is
  just `profile`, `auth`, and `version` (all that `/info` and `/version`
  expose today); the later capability-harvesting phase (§9) widens it to
  add `services` and `provides`.
...
receive-timeout clock; seed n0 + n1–n5; probe `/info` and `/version` only,
so the self-reported tier is `profile` + `auth` + `version` and **no
node-side endpoint change is required**; publish the generation-buffered
...
- **Probe transport.**  Plain HTTP `GET /info`, or an `rpc/2-3` call so the
